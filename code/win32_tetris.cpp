 ...
 *
 * - Make the right calls so Windows doesn't think we're "stil loading" for a bit after we actually start
...
Win32ToggleFullscreen(HWND Window)
{
    ...
    // for for fullscreen toggling, see:
...
// NOTE(kstandbridge): DirectSoundCreate
...
void 
DEBUGPlatformFreeFileMemory(thread_context *Thread, void *Memory)
{
    if (Memory)
    {
...
DEBUGPlatformReadEntireFile(thread_context *Thread, char *Filename)
{
    ...
    if (FileHandle != INVALID_HANDLE_VALUE)
    {
        ...
        {
            u32 FileSize32 = SafeTruncateSizeUInt64(FileSize.QuadPart);
...
b32 
DEBUGPlatformWriteEntireFile(thread_context *Thread, char *Filename, u32 MemorySize, void *Memory)
{
    b32 Result = false;
    ...
    if (FileHandle != INVALID_HANDLE_VALUE)
    {
        ...
        {
            // NOTE(kstandbridge): File written successfully
...
Win32ResizeDIBSection(win32_offscreen_buffer *Buffer, int Width, int Height)
{
    ...
    // NOTE(kstandbridge): When the biHeight field if negative, this is the clue to
    // Windows to treat this bitmap as top-down, not bottom-up, meaning that
    // the first three byte of the image are the color for the  top left pixel
...
Win32PlayBackInput(win32_state *Win32State, game_input *NewInput)
{
    ...
    if (ReadFile(Win32State->PlayBackHandle, NewInput, sizeof(*NewInput), &BytesRead, 0))
    {
        if (BytesRead == 0)
        {
            // NOTE(kstandbridge): We've hiut the end of the stream, go back to the beginning
...
{
    // TODO(kstandbridge): Draw where we're writing our sound
        ...
        int ShowCode)
{
    ...
    Win32BuildEXEPathFileName(&Win32State, "tetris.dll",
                              sizeof(SourceGameCodeDLLFullPath), SourceGameCodeDLLFullPath);
    
    char TempGameCodeDLLFullPath[WIN32_STATE_FILE_NAME_COUNT];
    Win32BuildEXEPathFileName(&Win32State, "tetris_temp.dll",
    ...
    b32 SleepIsGranular = (timeBeginPeriod(DesiredScheduleMS) == TIMERR_NOERROR);
    ...
    // We want dimensions to be a power of 2
    // width is fine because 1920 -> 2048 = 2048-1920 -> 128 pixels buffer left
    // height is a problem because 1080 -> 2048 = 2048 - 1080 -> 968 pixels left with is too much wasted
    // we can split height into two segments of 1024 + 128 = 1152  -> 128 pixels buffer left
	...
	if (RegisterClassA(&WindowClass))
	{
		...
		if (Window)
		{
			...
			// TODO(kstandbridge): How do we reliable query on this on Windows?
			...
			// On our machine, it was 960 samples.
			while(GlobalRunning)
			{
				DWORD PlayCursor;
				DWORD WriteCursor;
			...
			s16 *Samples = (s16 *)VirtualAlloc(nullptr, SoundOutput.SecondaryBufferSize,
            ...
            // call adjust token privileges when not on XP
            
            // TODO(kstandbridge: TransientStorage needs to be broken 
            // up into game game transient and cache transient, and only
                 ...
                 ++ReplayIndex)
			{
				...
				// on record start - find out what Windows is doing and if we can speed
				// up / defer some of that processing.
			...
			if (Samples && GameMemory.PermanentStorage && GameMemory.TransientStorage)
			{
				...
				while (GlobalRunning)
				{
					...
					// TODO(kstandbridge): We can't zero everything because the up/down state will be wrong!!!
					...
					if (!GlobalPause)
					{
						...
						for (DWORD ControllerIndex = 0; ControllerIndex < MaxControllerCount; ++ControllerIndex)
						{
							...
							if (XInputGetState(ControllerIndex, &ControllerState) == ERROR_SUCCESS)
							{
								...
								// TODO(kstandbridge): This is a square deadzone, check XInput to verify that the
								// deadzone is "round" and show to do round deadzone processing
						...
						r64 FromBeginToAudioSeconds = Win32GetSecondsElapsed(FlipWallClock, AudioWallClock);
						...
						if (GlobalSecondaryBuffer->GetCurrentPosition(&PlayCursor, &WriteCursor) == DS_OK)
						{
							 ...
							 * boundary plus on frame. This gives us perfect
							 ...
							 * worth of audio plus the safety margins worth
							 * of guard samples.
							 *
							 */
							...
							r32 SecondsLeftUntilFlip = (TargetSecondsPerFrame - (r32)FromBeginToAudioSeconds);
						...
						r64 MSPerFrame = 1000.0f * Win32GetSecondsElapsed(LastCounter, EndCounter);
